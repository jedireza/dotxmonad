...
-- | Set each screen to the respective workspaceId
-- | BUG: only works with two monitors
-- | BUG: only works when focused on the left screen
setWorkspaces :: [String] -> X ()
setWorkspaces workspaceIds = do
    windows $ W.greedyView $ head . drop 0 $ workspaceIds
    nextScreen
    windows $ W.greedyView $ head . drop 1 $ workspaceIds
    prevScreen
...
--
myWorkspaces = ["1:web","2:code","3:gimp","4:files"] ++ map show [5..9]
...
myKeys conf@(XConfig {XMonad.modMask = modMask}) = M.fromList $
  ...
  -- Workspace shortcuts
  -- Use this to change workspaces in pairs
  , ((modMask, xK_F1),
